...
3) (Optional - Works only on Liinux) Convert a "simple" xlsx file to pdf (support basic xlsx features for big pdf reports)
...
Compile (requires gcc that supports C++14 and on):

While on root:

cd .\libs\PDF-Writer-master\build
cmake .. && make
cd ..\..\..
make
...
# Write current page so far and create a new one
N
