...
{
	/*const auto node = this->cache.find(inString);

	if (node not_eq this->cache.end())
	{
		mStreamForWriting->Write(node->second.data(), node->second.size());
	...
	//std::basic_string<unsigned char> cachedString(scLeftParanthesis, 1);
	...
	for(;it != inString.end();++it)
	{
		...
		else
		{
			...
			//cachedString.push_back(aValue);
		}
		
	}
	mStreamForWriting->Write(scRightParanthesis,1);

	//cachedString.push_back(scRightParanthesis[0]);

	//this->cache[inString] = cachedString;
