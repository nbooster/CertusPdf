...
{
	//const auto node = this->glyphUnicodeMappingListCache.find(inUnicodeText);

	//if (node not_eq this->glyphUnicodeMappingListCache.end())
		//return WriteTextCommandWithDirectGlyphSelection(node->second, inTextCommand);

	GlyphUnicodeMappingList glyphsAndUnicode;
	EncodeWithCurrentFont(inUnicodeText, glyphsAndUnicode);

	//this->glyphUnicodeMappingListCache[inUnicodeText] = glyphsAndUnicode;

	return WriteTextCommandWithDirectGlyphSelection(glyphsAndUnicode, inTextCommand/*, inUnicodeText*/);
}
...
void AbstractContentContext::WriteText(double inX,double inY,const std::string& inText,const TextOptions& inOptions)
{
	...
	/*const auto node = this->glyphUnicodeMappingListCache.find(inText);

	if (node not_eq this->glyphUnicodeMappingListCache.end())
	{
		this->SetupColor(inOptions);
		this->BT();
		this->Tm(1,0,0,1,inX,inY);
		this->SetCurrentFontSize(inOptions.fontSize);
		this->Tj(node->second);
		this->ET();
	}
	...
	else
	{
		...
		this->glyphUnicodeMappingListCache[inText] = glyphsAndUnicode;
