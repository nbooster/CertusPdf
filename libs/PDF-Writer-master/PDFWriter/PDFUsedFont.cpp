...
EStatusCode PDFUsedFont::TranslateStringToGlyphs(const std::string& inText,GlyphUnicodeMappingList& outGlyphsUnicodeMapping)
{
	...
	/*const auto node = this->glyphCache.find(inText);

	if (node not_eq this->glyphCache.end())
		glyphs = node->second;
	...
	{*/
		...
		/*this->glyphCache[inText] = glyphs;
	}*/

	/*const auto tempNode = this->glyphUnicodeMappingListCache.find(inText);

	if (tempNode not_eq this->glyphUnicodeMappingListCache.end())
		outGlyphsUnicodeMapping = tempNode->second;
	...
	{*/
		...
		/*this->glyphUnicodeMappingListCache[inText] = outGlyphsUnicodeMapping;
	}*/
...
{
	const std::lock_guard<std::mutex> lock(this->textMutex);

	const auto tit = (this->textCache).find(inText);

	if (tit not_eq (this->textCache).end())
		return tit->second;
	...
	else
	{
	    ...
	    unicode.FromUTF8(inText);
		mFaceWrapper.GetGlyphsForUnicodeText(unicode.GetUnicodeList(),glyphs);
		const auto temp = CalculateTextDimensions(glyphs,inFontSize);

		(this->textCache).emplace(inText, temp);
...
PDFUsedFont::TextMeasures PDFUsedFont::CalculateTextDimensions(const UIntList& inGlyphsList, long inFontSize)
{
    ...
    for(; it != inGlyphsList.end();++it)
    {
        ...
        {
        	const std::lock_guard<std::mutex> lock(this->widthMutex);

	        const auto wit = (this->widthCache).find(temp);

	        if (wit not_eq (this->widthCache).end())
	        	pen_x += wit->second;

	        else{
	        	const auto width = mFaceWrapper.GetGlyphWidth(temp);

	        	(this->widthCache).emplace(temp, width);
	        	
	        	pen_x += width; //mFaceWrapper.GetGlyphWidth(*it);
	        }
	    }
    }
    ...
    for(; it != inGlyphsList.end();++it,++itPos)
    {
    	...
    	{
    		const std::lock_guard<std::mutex> lock(this->bboxMutex);

	    	const auto bit = (this->bboxCache).find(temp);

	    	if (bit not_eq (this->bboxCache).end())
	        	glyph_bbox = bit->second;

	        else{
				mFaceWrapper.LoadGlyph(mFaceWrapper.GetGlyphIndexInFreeTypeIndexes(temp));
		        FT_Glyph aGlyph;
		        ...
		        (this->bboxCache).emplace(temp, glyph_bbox);
		    }
		}
