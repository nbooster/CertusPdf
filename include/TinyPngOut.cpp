...
TinyPngOut::TinyPngOut(uint32_t w, uint32_t h, /*std::ostream &out,*/ std::vector<unsigned char>& outVector) :
		// Set most of the fields
		width(w),
		height(h),
		//output(out),
		vectorPosition(outVector.data()),
		positionX(0),
		positionY(0),
		deflateFilled(0),
		adler(1) {
	...
	/*write(header);*/ 
	//outVector.insert( outVector.end(), std::begin(header), std::end(header) );
	std::memcpy(vectorPosition, header, 43);
	vectorPosition += 43;
...
void TinyPngOut::write(const uint8_t pixels[], size_t count/*, std::vector<unsigned char>& outVector*/) {
	if (count > SIZE_MAX / 3)
	...
	while (count > 0) {
		...
		if (deflateFilled == 0) {  // Start DEFLATE block
			...
			/*write(header);*/ 
			//outVector.insert( outVector.end(), std::begin(header), std::end(header) );
			std::memcpy(vectorPosition, header, 5);
			vectorPosition += 5;
		...
		if (positionX == 0) {  // Beginning of line - write filter method byte
			uint8_t b[] = {0};
			/*write(b);*/ 
			//outVector.insert( outVector.end(), std::begin(b), std::end(b) );
			std::memcpy(vectorPosition, b, 1);
			vectorPosition += 1;
			...
			//outVector.insert( outVector.end(), pixels, pixels + n );
			std::memcpy(vectorPosition, pixels, n);
			vectorPosition += n;
		...
		if (positionX == lineSize) {  // Increment line
			...
			if (positionY == height) {  // Reached end of pixels
				...
				/*write(footer);*/ 
				//outVector.insert( outVector.end(), std::begin(footer), std::end(footer));
				std::memcpy(vectorPosition, footer, 20);
				vectorPosition += 20;
			}
		}
	}
