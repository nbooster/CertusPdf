...
bool QrToPng::writeToPNG(std::vector<unsigned char>& outVector) {
    /* text is required */
    if (_text.empty())
        return false;
    ...
    try {
        ...
        auto result = _writeToPNG(qrcodegen::QrCode::encodeText(_text.c_str(), _ecc), outVector);
...
bool QrToPng::_writeToPNG(const qrcodegen::QrCode &qrData, std::vector<unsigned char>& outVector) const {
    //std::ofstream out(_fileName.c_str(), std::ios::binary);
    int pngWH = _imgSizeWithBorder(qrData);
    TinyPngOut pngout(pngWH, pngWH, /*out,*/ outVector);
    ...
    pngout.write(tmpData.data(), static_cast<size_t>(tmpData.size() / 3)/*, outVector*/);
    tmpData.clear();
    ...
    return true;//fs::exists(_fileName);
}
