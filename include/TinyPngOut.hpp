...
class TinyPngOut final {
	...
	private: unsigned char* vectorPosition = nullptr;
	 ...
	 */
	public: explicit TinyPngOut(std::uint32_t w, std::uint32_t h, /*std::ostream &out,*/ std::vector<unsigned char>& outVector);
	 ...
	 */
	public: void write(const std::uint8_t pixels[], size_t count/*, std::vector<unsigned char>& outVector*/);
